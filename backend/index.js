...
app.post("/upload", upload.array("files", 5), (req, res) => {
  ...
  return res
    .status(200)
    .json({ message: "Files uploaded and deleted successfully" });
});
  ...
  try {
    if (Object.values(search).join()) {
      ...
      const films = await db.Films.findAll({
        include: [
          { model: db.Buy_links, as: "buy_links" },
...
app.delete("/api/press/:id", async (req, res) => {
  ...
  try {
    const response = await db.Press.destroy({ where: { id: pressId } });
    if (!response) {
      return res.status(404).send("Press not deleted");
    }
  ...
  try {
    if (title) {
      ...
      const video = await db.Videos.findOne({
        where: {
          [Op.and]: [{ filmId: film.id }, { featured: true }],
        },
      });
      res.send(video);
    }
...
app.post("/api/film/homeSet", ensureAuthenticated, async (req, res) => {
  ...
  try {
    ...
    await db.Videos.update(
      { featured: false },
      { where: { [Op.and]: [{ featured: true }, { filmId: film.id }] } }
    );
    videoUpdated = await db.Videos.update(
      { featured: true },
      { where: { [Op.and]: [{ video: url }, { filmId: film.id }] } }
    );
...
app.get("/api/feature/delete/film", ensureAuthenticated, async (req, res) => {
  ...
  try {
    if (id) {
      const removedFeature = await db.Films.update(
        { homeId: null },
        { where: { id: id } }
      );
      await db.Videos.update(
        ...
        {
          where: {
            [Op.and]: [{ filmId: id }, { featured: true }],
          },
        }
      );
...
app.post(
  "/api/homepage/film/create/video",
  ensureAuthenticated,
  async (req, res) => {
...
app.post("/api/play/create/video", ensureAuthenticated, async (req, res) => {
  ...
  try {
    const play = await db.Plays.findOne({
      where: { title: vidAdd.title },
      attributes: ["id", "title"],
    });
    const video = await db.Videos.create({
